...
Processed_Dataset,Train,Test = atl.atlantic_data_processing(Dataset,                      # Dataset:pd.DataFrame, 
                                                        ...
                                                        h2o_fs_models=7,              # vif_ratio:float=10.0 
                                                        encoding_fs="on",
                                                        vif_ratio=10.0)
```  
...
from atlantic import Atl_Data_Processing
...
* [H2O AutoML Feature Selection](https://docs.h2o.ai/h2o/latest-stable/h2o-docs/variable-importance.html) - This function is based of how variable importance is calculated for tree-based models in H2O’s AutoML and it can be customized by use of the following parameters: 
...
Reg_Performance = pd.DataFrame(metrics_regression(y_true,y_pred),index=[0])    # y_true:list, y_pred:list
    
Binary_Class_Performance = pd.DataFrame(metrics_binary_classification(y_true,y_pred),index=[0])    # y_true:list, y_pred:list
    
MultiClass_Performance = pd.DataFrame(metrics_classification(y_true,y_pred),index=[0])    # y_true:list, y_pred:list
